...
function configure_insecure_registry() {
    ...
    # Use jq to add insecure registry
...
if [[ "$is_manager" =~ ^[Yy]$ ]]; then
    ...
    # Optional: Prompt to create local registry on manager
    ...
    # Worker can also set up insecure registry if needed
...
######################################
# Prompt to configure insecure registry if not already done
